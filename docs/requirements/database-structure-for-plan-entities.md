...
# Physical Data Model not for MVP
...
| `created_at`        | TIMESTAMPTZ         | Timestamp when the intensity record was created                   | Not null                                        | CURRENT_TIMESTAMP |
...
| `created_at`     | TIMESTAMPTZ        | Timestamp when the duration record was created        | Not null                                           | CURRENT_TIMESTAMP |
...
| `updated_at`        | TIMESTAMPTZ        | Timestamp of last update                                 | Not null                                                | CURRENT_TIMESTAMP |
| `completed_at`      | TIMESTAMPTZ        | Timestamp when the task was completed                    | Nullable                                                | NULL              |     |
| `is_custom`         | BOOLEAN            | Whether task is created by system or user                | Not null                                                | FALSE             |
| `parent_task_id`    | INT                | Optional reference to the task this was regenerated from | FK → `task(id)`, ON DELETE SET NULL                     | NULL              |

# 7. Table: task_translation
...
| Column          | Type         | Description                                             | Constraints                                                                                   | Default        |
| --------------- | ------------ | ------------------------------------------------------- | --------------------------------------------------------------------------------------------- | -------------- | ---- |
| `id`            | SERIAL       | Unique identifier for each translation record           | Primary Key                                                                                   | Auto-increment |
| `task_id`       | INT          | Reference to the original task being translated         | **NOT NULL**, Foreign Key → `task(id)`, **ON DELETE CASCADE** (deletes translation with task) | None           |
| `language_code` | VARCHAR(5)   | ISO language code for the translation (e.g. `en`, `es`) | **NOT NULL**, Foreign Key → `language(code)`                                                  | None           |
| `title`         | VARCHAR(200) | Translated task title                                   | **NOT NULL**                                                                                  | None           |
| `description`   | TEXT         | Translated description for the task                     | **NOT NULL**                                                                                  |                | NULL |
...
| `execution_time` | `task`      | `execution_time_id` | ON DELETE SET NULL | Set execution_time_id to NULL if deleted |
